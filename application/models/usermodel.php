...
class Usermodel extends CI_Model
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    public function createUser($username, $version, $loginId, $age, $gender, $avatar, $group, $email="")
    {
        $data = array(
            'name' => $username,
            'email' => $email,
    ...
    public function updateUser($username, $version, $age, $gender, $avatar, $group, $email, $level,$uid)
    {
        $data = array(
            'name' => $username,
            'email' => $email,
...
//        if($this->db->error)
    ...
    public function getUserIdFromLogin($loginUid)
    {
        $this->db->select("id");
        $this->db->where("loginId", $loginUid);
        $this->db->from("user");
        $query = $this->db->get();
        $result = $query->result_array();
        if ($query->num_rows() > 0) {
    ...
    public function getTopxtillyWithFriends($from, $to, $isRecent = 0, $requesterUserId,$useFriends =false)
    {
        if ($isRecent == 1)
            $this->db->select("user.id, username, progressLevel, recentScore");
        else
            $this->db->select("user.id, username, progressLevel, score");

        $this->db->from("user");
        if($useFriends){
            $this->db->join('followings','user.id = followings.followee_id');
            $this->db->where('follower_id',$requesterUserId);
        }
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    public function leaderboardShouldReset()
    {
        ...
        if ($query->num_rows() > 0) {
            $lastLeaderboardFinishTime = $result[0]["finishTime"];
            $tempLeagueDays = 3600 * 24 * 3;

            if (time() >= intval($lastLeaderboardFinishTime))
                return $lastLeaderboardFinishTime;
            else
                return -1;
        } else
            return -1;
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //    
    public function resetLeagueLeaderboard($lastLeaderboardFinishTime)
    {
        ...
        if ($query->num_rows() > 0) {
            ...
            $difference = time() - $lastLeaderboardFinishTime;

            $newFinishTime = (floor($difference / $tempLeagueDays) + 1) * $tempLeagueDays + $lastLeaderboardFinishTime;

            $data = array("setTime" => strval(time()), "finishTime" => strval($newFinishTime));
    ...
    public function getDesiredGameAdaption($userPersonalityType)
    {
        ...
        if ($snPart == "N") {
            $this->db->like("pType", $snPart . $tfPart);
        } else {
            $this->db->where("pType", "I" . $snPart . "T" . $pjPart);
            $this->db->or_where("pType", "I" . $snPart . "F" . $pjPart);
            $this->db->or_where("pType", "E" . $snPart . "T" . $pjPart);
            $this->db->or_where("pType", "E" . $snPart . "F" . $pjPart);
        }
        ...
        $checkArray = array(
            array("badgeonoff" => 0, "leaderBoardonoff" => 0, "feedBackonoff" => 0),
            array("badgeonoff" => 1, "leaderBoardonoff" => 0, "feedBackonoff" => 0),
            array("badgeonoff" => 0, "leaderBoardonoff" => 1, "feedBackonoff" => 0),
            array("badgeonoff" => 0, "leaderBoardonoff" => 0, "feedBackonoff" => 1),
            array("badgeonoff" => 1, "leaderBoardonoff" => 1, "feedBackonoff" => 1)
        );

        $checkCountArray = array(0, 0, 0, 0, 0);

        if ($query->num_rows() > 0) {
            for ($j = 0; $j < count($checkArray); $j++) {
                for ($i = 0; $i < $query->num_rows(); $i++) {
                    if ($result[$i]["badgeonoff"] == $checkArray[$j]["badgeonoff"] &&
                        $result[$i]["leaderBoardonoff"] == $checkArray[$j]["leaderBoardonoff"] &&
                        $result[$i]["feedBackonoff"] == $checkArray[$j]["feedBackonoff"]
                    ) {
                        $checkCountArray[$j] += intval($result[$i]["usercount"]);
                    }
                }
            }
        }
        ...
        $desireAdaption = -1;
        for ($j = 0; $j < count($checkArray); $j++) {
            if ($checkCountArray[$j] < $checkUserCount || $checkUserCount == -1) {
                $checkUserCount = $checkCountArray[$j];
                $desireAdaption = $j;
            }
        }

        $badgeonoff = $checkArray[$desireAdaption]["badgeonoff"];
        $leaderBoardonoff = $checkArray[$desireAdaption]["leaderBoardonoff"];
        $feedBackonoff = $checkArray[$desireAdaption]["feedBackonoff"];
        ...
        if ($query->num_rows() > 0) {
            $result[0]['isControl'] = 2; //$isControl;
            return $result[0];
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //
    public function setUserQuestionarieTimes($uid, $userResultTime)
    {
        $data = array('uid' => $uid,
            'times' => strval($userResultTime)
        );

        $this->db->insert('questionarietimes', $data);
    }

    /**
     * This method searches for the users matching the given name (or part of name)
     * @param $name part of username
     * @param $uid id of current user
     * @param int|\specifies $limit specifies how many users to return, default =5
     * @return string result if found, -1 if not found
     */
    public function searchUsersByName($name, $uid, $limit = 5)
    {
        $this->db->select("id");
        $this->db->from("user");
        ...
        if ($query->num_rows() > 0) {
            foreach ($result as $res) {
                $resultString .= implode($this->getUserMinimalData($res['id'])[0], ',');
                $resultString .= ',' . $this->followModel->checkFollow($uid, $res['id']) . ',';
            }
            return $resultString;
        } else
            return -1;
    ...
    /**
     * This function returns username and level of given user
     * @param $uid
     * @return array, or -1 if user not found
     */
    public function getUserMinimalData($uid)
    {
        $this->db->select("id,progressLevel, username, score");
        $this->db->from("user");
        $this->db->where("id", $uid);
        $query = $this->db->get();
        $result = $query->result_array();
    ...
    /**
     * Inserts a row in "profileviews" db table
     * @param $viewer user who viewed a prifile
     * @param $viewed user whose pr
     * @return int 0 indicating success (?)
     */
    public function setProfileView($viewer, $viewed)
    {
        $data = array(
            "viewerid" => $viewer,
            "viewedid" => $viewed
        );
        $this->db->insert('profileviews', $data);
        return 0;
    }

    public function setDailyGoal($uid, $dg)
    {
        $this->db->where("id", $uid);
        $this->db->set('dailyGoal', $dg);
        $this->db->update('user');
    }

    public function getDailyGoal($uid)
    {
        $this->db->select("dailyGoal");
        $this->db->from("user");
        $this->db->where("id", $uid);
        $query = $this->db->get();
        $result = $query->result_array();
        return ($result[0]['dailyGoal']);
    }
}
